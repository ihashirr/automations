...
export function CaptureMapPicker({ initialCoordinates, onClose, onConfirm, visible }: CaptureMapPickerProps) {
  ...
  async function handleConfirm() {
    if (!markerCoordinates || isConfirming) return;
    setIsConfirming(true);
    try {
      await onConfirm(markerCoordinates);
    } finally {
      ...
      <View style={styles.container}>
        {markerCoordinates ? (
          <MapView
            ...
            onPress={({ nativeEvent }) => setMarkerCoordinates({ lat: nativeEvent.coordinate.latitude, lng: nativeEvent.coordinate.longitude })}
            showsCompass={false}
          ...
          >
            <Marker
              coordinate={{ latitude: markerCoordinates.lat, longitude: markerCoordinates.lng }}
              draggable
              onDragEnd={({ nativeEvent }) => setMarkerCoordinates({ lat: nativeEvent.coordinate.latitude, lng: nativeEvent.coordinate.longitude })}
            />
          </MapView>
        ) : (
        ...
        <View style={[styles.bottomCard, { paddingBottom: insets.bottom + spacing.lg }]}>
          ...
          <Text style={styles.helper}>Tap the map or drag the pin, then confirm the location.</Text>
